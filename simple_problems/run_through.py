...
def main():
    ...
    mu_= jr.uniform(key1, shape=(10000,dim), minval=MIN_MU, maxval=MAX_MU)
    theta_ = mu_

    # make test set
    key1,key2 = jr.split(key1)
    mu_test = jr.uniform(key1, shape=(5000,dim), minval=MIN_MU, maxval=MAX_MU)
    ...
    hids = jnp.array([10, 10, 10, 20, 20, 20, 32, 50, 50, 50, 50, 64, 64, 100, 100, 100, 128])
...
# ---------------------------------------------------------------------------------------------
    ...
    plt.scatter(mus_true, mus_pred, label="data", s=2)
